...
- `image-budget.ts` — Outgoing request-body budget. Ollama Cloud rejects bodies over 16 MiB with `400 failed to read request body`; `trimImagesToBudget` de-duplicates identical inline images (newest copy kept), then replaces the oldest remaining images with placeholder text until the serialized OpenAI-completions payload fits, resolving a replaced image's source path (toolCall `arguments.path` -> `tool_call_id` -> image message, or `<file name>` tags from `@file`) so the placeholder can tell the model to re-read it. Pure and unit-tested; `index.ts` wires it to the `before_provider_request` hook and `config.ts` supplies the `maxRequestBytes` budget. Do not mutate the input payload in place - pi reuses it across retries.
