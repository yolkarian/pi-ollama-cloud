...
- Fix vision sessions failing with `400 failed to read request body` once the conversation accumulates more than 16 MiB of inline images. Ollama Cloud rejects any request body above its 16 MiB cap before parsing, and pi re-sends every historical image on each turn, so past the cap every turn failed regardless of the prompt. A `before_provider_request` hook now de-duplicates identical inline images (keeping the newest copy) and then replaces the oldest remaining images with placeholder text (keeping the newest) until the serialized body fits the budget; when an image came from a file, the placeholder carries the source path (`[image omitted; re-read with read /path.png]`) so the model can re-read it on demand. Configure with `"maxRequestBytes"` in `ollama-cloud.json` (default 14 MiB, clamped to Ollama's 16 MiB hard limit).
